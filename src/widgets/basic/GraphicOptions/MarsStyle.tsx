...
export default function MarsStyle({ style, customType, graphicType, onChange = () => {} }: MarsAttrProps) {
  ...
  function getViewShow(config, styleOptions) {
    if (typeof config.show === "function") {
      return config.show(styleOptions, graphicType)
    }
    return true
  }
  ...
  function setDefault(newStyleValue) {
    ...
    originStyles.current = [...styleConfig.style]
  ...
  function setLabelDefault(newStyleValue) {
    originLabels.current = [...styleConfigAll?.label?.style]
